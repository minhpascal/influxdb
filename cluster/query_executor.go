...
type StatementExecutor struct {
	...
	// Holds query management information.
...
func (e *StatementExecutor) ExecuteStatement(stmt influxql.Statement, ctx *influxql.ExecutionContext) error {
	...
	if stmt, ok := stmt.(*influxql.SelectStatement); ok {
		return e.executeSelectStatement(stmt, ctx.ChunkSize, ctx.StatementID, ctx.QueryID, ctx.Results, ctx.InterruptCh)
	}
...
func (e *StatementExecutor) executeSelectStatement(stmt *influxql.SelectStatement, chunkSize, statementID int, qid uint64, results chan *influxql.Result, closing <-chan struct{}) error {
